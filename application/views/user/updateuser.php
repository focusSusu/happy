        ...
        <div class="box listPage">
            <form class="form-horizontal" id="CreateForm">
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" name="username" class="form-control" value="<?= $userinfo['username'] ?>" id="name" placeholder="">
                    </div>
                </div>
                <div class="form-group">
                    <label for="name" class="col-sm-3 control-label">真实姓名</label>
                    <div class="col-sm-8">
                        <input type="text" name="real_name" class="form-control" value="<?= $userinfo['real_name'] ?>" id="name" placeholder="">
                    </div>
                </div>
<!--                <div class="form-group">-->
                        ...
                        <div class="checkbox">
                            <?php if($userinfo['is_admin'] == 1): ?>
                                ...
                                <?php foreach ($menus as $k=>$v): ?>
                                    <label>
                                        <input type="checkbox" name="permis_id[]" value="<?= $v['id'] ?>" lay-skin="primary" disabled title="<?= $v['name'] ?>"
                                            <?php if(in_array($v['id'],explode(',',$userinfo['permis_id']))): ?>
                                                checked=""
                                            <?php endif; ?>
                                        ><?= $v['name'] ?>
                                    </label>
                            ...
                            <?php else: ?>
                                <?php foreach ($menus as $k=>$v): ?>
                                    <label>
                                        <input type="checkbox" name="permis_id[]" value="<?= $v['id'] ?>" lay-skin="primary" title="<?= $v['name'] ?>"
                                            <?php if(in_array($v['id'],explode(',',$userinfo['permis_id']))): ?>
                                                checked=""
                                            <?php endif; ?>
                                        ><?= $v['name'] ?>
...
<script>
    $(function () {
        $("#sub").click(function () {
            ...
            var url = "<?= base_url()."user/updateUser/" ?>";
            $.ajax({
                type:"post",
                url:url,
                processData:false,
